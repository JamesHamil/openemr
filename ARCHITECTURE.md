...
## Defense Answer in One Paragraph

This architecture is safer than an all-PHP in-app agent because it isolates experimental AI behavior from OpenEMR's core clinical runtime while preserving OpenEMR as the only authority for identity, patient context, authorization, direct clinical-data access, and patient-linked audit events. It is safer than an external chatbot because it is committed inside the OpenEMR fork, remains embedded in the EHR workflow, and does not create a second unmanaged EHR or clinical data surface. The FastAPI sidecar is an isolated AI runtime, not a data authority: it receives signed, short-lived, minimum-necessary evidence bundles from OpenEMR, cannot call back for more chart data, and returns only verified output. The design is credible under a one-day deadline because the first slice can be mocked behind the real request and response contracts while still proving the essential pattern: read-only access, OpenEMR-owned evidence retrieval, source-bound claims, verification before display, observable execution, and honest failure behavior.

## Presentation Guide

### 1. Start With the Problem

The clinical problem is not that doctors need another chatbot. The problem is that a hospitalist often has less than two minutes before entering a patient room and needs to quickly understand what changed, what matters, and what the chart actually supports.

In a clinical setting, a fluent unsupported answer is dangerous. The core question for this architecture is therefore not "can the model answer?" It is "can the system prove what it says, and can it fail safely when it cannot?"

### 2. Explain the User and the Narrow V1

The first user is a hospitalist preparing for inpatient rounds. V1 supports three narrow workflows:

1. A source-backed chart brief for one selected patient.
2. Follow-up questions inside that same patient context.
3. Safe refusal or reframing when the user asks for treatment decisions.

This is intentionally read-only. The assistant does not write notes, place orders, change medications, update diagnoses, create billing records, or assign tasks. It helps the physician understand retrieved chart evidence faster, but the physician remains the decision-maker.

### 3. Name the Key Architectural Choice

The main design decision is an in-repo hybrid sidecar.

OpenEMR remains the trusted clinical system. It owns login, user identity, browser session, selected patient, encounter context, CSRF checks, ACL checks, audit events, and direct access to clinical records.

The AI sidecar is a separate FastAPI runtime inside the same OpenEMR fork. It owns model orchestration, structured output parsing, verification, evals, and operational tracing.

The important distinction is that the sidecar is not a second EHR. It has no OpenEMR database credentials, no OpenEMR API token, no delegated clinical tools, and no callback channel to fetch more chart data.

### 4. Explain the Evidence Bundle

The strongest safety boundary is the `RoundingContextBundle`.

Before the sidecar is called, OpenEMR builds a bounded evidence bundle using OpenEMR-owned services, controllers, or narrow adapters. That bundle contains only the patient evidence the current user is authorized to see. It also records which evidence collectors succeeded, failed, timed out, or are not implemented.

The sidecar receives this bundle and only this bundle. It may transform the evidence into a concise answer, but it may not fetch, expand, cache, or authorize clinical data.

This keeps clinical authority in OpenEMR while still allowing the AI runtime to be isolated and testable.

### 5. Walk Through the Request Flow

The flow is:

1. The hospitalist opens a patient chart in OpenEMR.
2. The browser sends a chat request to the AgentForge OpenEMR module.
3. OpenEMR validates session, CSRF, role, patient access, encounter context, and rate limits.
4. OpenEMR builds an ACL-filtered `RoundingContextBundle`.
5. OpenEMR signs a short-lived request and sends it to the sidecar.
6. The sidecar calls OpenAI using structured output.
7. The verifier checks every factual claim against the evidence bundle.
8. The sidecar returns a verified, partial, refused, or failed response.
9. OpenEMR records a patient-linked audit event and renders the answer with citations.

For engineers, the essential pattern is: OpenEMR is the context broker; the sidecar is the evidence transformer.

### 6. Explain Verification

Verification is not just "the answer has citations." A hallucinated sentence can cite a real record and still be wrong.

For that reason, each factual claim must include:

- A source ID.
- A source record type.
- A source field path or note span.
- An extracted value.
- A timestamp when available.

The verifier checks whether the cited source actually supports the claim. Unsupported claims are removed, rewritten as uncertainty, or refused. Conflicting sources are surfaced as conflicts, not resolved by model preference.

The response status tells the UI and the physician what happened:

- `verified`: all factual claims are supported.
- `partial`: the answer is verified against available evidence, but some data was missing, unavailable, or timed out.
- `refused`: the request is unsafe, unauthorized, or asks for a treatment directive.
- `failed`: the system could not produce a controlled verified answer.

### 7. Explain Safety and Failure Behavior

The system is designed to fail visibly.

If labs are missing from the retrieved bundle, the assistant says it does not see those labs in retrieved records. It does not say the labs were not done.

If pending orders are not supported by a v1 adapter, the assistant says pending orders were not included in the evidence bundle. It does not say there are no pending orders.

If the model returns malformed output, the sidecar retries once with the schema. If it still fails, the system returns a controlled error.

If chart text contains prompt injection, it is treated as untrusted patient-record content. It cannot override system policy, OpenEMR authorization, or verifier rules.

### 8. Explain Observability and Audit

There are two linked records for each request.

OpenEMR records the patient-linked clinical audit event. That belongs in the EHR because OpenEMR is the system of record.

The sidecar records a PHI-safe operational trace. That trace uses hashes and bundle IDs rather than raw chart identifiers. It tracks collector latency, model name, prompt version, tokens, estimated cost, verification result, blocked claims, and final status.

The trace ID links the two records without turning the sidecar into a PHI log store.

### 9. Explain Why Not the Alternatives

An all-PHP agent would be simpler to deploy, but it would mix experimental AI behavior into the EHR runtime and make structured model orchestration, evals, and tracing harder.

A standalone chatbot would be easier to demo, but it would not naturally inherit OpenEMR's user session, patient context, ACLs, or audit model. It would risk becoming a second unmanaged clinical data surface.

The hybrid sidecar is the compromise: OpenEMR stays authoritative, while AI behavior is isolated behind a narrow, testable runtime boundary.

### 10. End With the Defense

The defensible claim is:

This is not a broad medical chatbot. It is a read-only, patient-scoped, source-backed chart assistant. OpenEMR decides who can see what. OpenEMR builds the evidence bundle. The sidecar can only transform that evidence. Every clinical claim must survive verification before display. If the system cannot prove a statement, it must say less, warn clearly, or refuse.

That is the architecture we can safely build on.
